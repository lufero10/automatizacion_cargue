...
def main(params_pyt=None):
    ...
    if params_pyt:
        # Si viene de la .pyt, usamos los valores que ella nos mande
        ruta_excel = params_pyt[0]
        nombre_hoja = params_pyt[1]
        tematica = params_pyt[2].lower()
        inputGeom = params_pyt[3]
        route = params_pyt[4]
        tolerancia = int(params_pyt[5]) if params_pyt[5] else 50
        gdb_destino = params_pyt[6]
    else:
        # Si se ejecuta como script tool normal, sigue usando GetParameterAsText
        ruta_excel = arcpy.GetParameterAsText(0)
        nombre_hoja = arcpy.GetParameterAsText(1)
        tematica = arcpy.GetParameterAsText(3).lower()
        inputGeom = arcpy.GetParameterAsText(3)
        route = arcpy.GetParameterAsText(4)

        val_tol = arcpy.GetParameterAsText(5)
        tolerancia = int(val_tol) if val_tol else 50
    ...
    print("📊 [2/6] Validando calidad de datos en el Excel...")
    arcpy.AddMessage("📊 [2/6] Validando calidad de datos en el Excel...")

    df = pd.read_excel(ruta_excel, sheet_name=nombre_hoja)
    informe = generar_informe_validacion(df, mapeo_tematica)

    # Solo imprimimos si hay errores de calidad reales
    if informe["errores_adicionales"]:
        print("⚠️ ADVERTENCIAS DE CALIDAD ENCONTRADAS:")
        for error in informe["errores_adicionales"]:
            print(f"  - {error}")
            arcpy.AddWarning(error)
    else:
        print("✅ Calidad de datos verificada (Contrato y Fechas OK).")
        arcpy.AddMessage("✅ Calidad de datos verificada (Contrato y Fechas OK).")

    # Ya no es necesario el pprint(informe) gigante porque ya filtramos lo útil
    print("✅ Paso de validación finalizado.\n")
